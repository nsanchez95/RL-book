...
class RiskyRetDistr(Distribution[A]):
...
class AssetAllocDiscrete:
    ...
    risky_return_distributions_params: Sequance[Tuple[float,float]]
    ...
    def get_mdp(self, t: int) -> MarkovDecisionProcess[float, float]:
        ...
        distr_mean: float =  self.risky_return_distributions_params[t][0]
        distr_var: float =  self.risky_return_distributions_params[t][1]
...
if __name__ == '__main__':
    ...
    utility_function_exp: Callable[[float], float] = lambda x: - np.exp(-a * x) / a
