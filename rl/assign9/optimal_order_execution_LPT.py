...
class OptimalOrderExecution:
    ...
    market_factor_dynamics: Sequence[Callable[[PriceAndShares], Distribution[float]]]
        ...
        class OptimalExecutionMDP(MarkovDecisionProcess[PriceAndShares, int]):
            ...
            def step(
                ...
                def sr_sampler_func(
                    ...
                    next_market_factor = market_factor_dynamics[t](p_s).sample()
...
if __name__ == '__main__':
    ...
    # Q
    price_diff = [lambda p_s: \
        p_s.price*(beta * p_s.shares + theta*p_s.market_factor) for _ in range(num_time_steps)]
    ...
    # X_t+1
    market_factor_dynamics = [lambda p_s: Gaussian(
        μ=p_s.market_factor*rho,
        σ=sigma_eta
    ) for _ in range(num_time_steps)]

    ffs = [
        lambda p_s: p_s.price,
        lambda p_s: p_s.price * p_s.shares,
        lambda p_s: p_s.price * p_s.market_factor,
        lambda p_s: p_s.price * float(p_s.shares)**2,
        lambda p_s: p_s.price * float(p_s.market_factor)**2,
        lambda p_s: p_s.price * p_s.market_factor * float(p_s.shares)
    ]
    ...
    ooe: OptimalOrderExecution = OptimalOrderExecution(
        ...
        market_factor_dynamics = market_factor_dynamics,
