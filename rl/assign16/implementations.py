...
from rl.markov_decision_process import FinitePolicy, Policy, StateActionMapping,MarkovDecisionProcess, TransitionStep
from rl.markov_process import FiniteMarkovProcess, FiniteMarkovRewardProcess
from rl.distribution import Categorical, Constant, Distribution, Choose
from rl.function_approx import FunctionApprox, Tabular, DNNApprox
...
class ApproxLogitPolicy(Policy[S, A]):
    ...
    def act(self, s: S) -> Optional[Distribution[A]]:
        if mdp.is_terminal(s):
            return None
        logits = np.array([logit_approx((s,a)) for a in mdp.actions(s)])
        prob_dist = np.exp(logits)
        prob_dist /= np.sum(prob_dist)
        return Categorical({a : prob_dist[i] for i, a in enumerate(mdp.actions(s))})

def REINFORCE(
    mdp: MarkovDecisionProcess,
    states: Distribution[S],
    pi_approx : DNNApprox[Tuple[S,A]],
    gamma: float,
            ):
    p = ApproxLogitPolicy(mdp = mdp, logit_approx = function_approx)
    trace_count = 1
    while True:
